...
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "psychological_bot_project.settings")
...
from psychological_companion.models import ChatMessage, save_chat_message, EmotionEntry


sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), 'psychological_bot_project')))
...
async def accept_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if user_id not in active_chats:
            ...
            # Обновляем словарь connections, чтобы связать пользователя с психологом
            connections[user_id] = psychologist_id
...
async def handle_user_message(update: Update, context):
    ...
    if user_id in connections:
        ...
        # Сохранить историю чата в базе данных (подставьте здесь вашу логику)
        save_chat_message(user_id, psychologist_id, message_text)

async def handle_psychologist_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    psychologist_id = update.effective_user.id
    message_text = update.message.text

    # Получить идентификатор пользователя из словаря connections
    user_id = connections.get(psychologist_id)
    ...
    if user_id:
        ...
        await context.bot.send_message(
            chat_id=psychologist_id,
            text="Пожалуйста, начните чат с команды /chat."
        )
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_id in connections:
        # Если пользователь находится в анонимном чате, удалите его из connections и active_chats
        psychologist_id = connections[user_id]

        # Отправьте сообщение психологу о завершении чата
        await bot.send_message(chat_id=psychologist_id, text=f"Пользователь {user_id} завершил чат.")

        # Удалите запись о пользователе из connections и active_chats
        del connections[user_id]
        del active_chats[user_id]

        await context.bot.send_message(chat_id=update.effective_chat.id, text=f"Чат завершен. Пока, {user.first_name}!")
    else:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="Вы не находитесь в анонимном чате.")

    # Возвращаем состояние ConversationHandler.END, чтобы завершить текущий разговор
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.USER & filters.TEXT, handle_user_message))
    application.add_handler(
        MessageHandler(filters.TEXT, handle_psychologist_message))
